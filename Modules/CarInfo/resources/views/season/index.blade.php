...
@section('meta_title', __('admin.common.seasons') . ' || ' . $companyName)

@section('content')
<!-- Page Wrapper -->
<div class="page-wrapper">
    <div class="content me-4">
        <x-admin.breadcrumb :title="__('admin.common.seasons')" :breadcrumbs="[
                    __('admin.common.seasons') => ''
                ]" :buttonText="__('admin.rentals.add_new_season')" :modalId="'add_season'" :buttonId="'add_new_season'"
        ...
        <div class="custom-datatable-filter table-responsive brandstable d-none real-table">
            <table class="table" id="seasonTable">
                <thead class="thead-light">
                    <tr>
                        <th>{{ strtoupper(__('admin.rentals.season_name')) }}</th>
...
<x-admin.modal className="addmodal" id="add_season" :title="__('admin.rentals.create_season')"
    action="{{  route('store_season') }}" formId="seasonForm" method="POST">
    <x-slot name="body">
        <input type="hidden" name="id" id="id">
        <div class="mb-3">
            <label for="name" class="form-label">{{ __('admin.rentals.season_name') }}<span class="text-danger">
                    *</span></label>
            ...
            <div class="form-check form-check-md form-switch me-2 d-none" id="statusDiv">
                <label for="status" class="form-check-label form-label mt-0 mb-0">
                    <input class="form-check-input form-label me-2 status" name="status" id="status" type="checkbox"
                        role="switch" checked aria-checked="false">
...
@push('scripts')
<script src="{{ asset('backend/assets/js/vehicleinfo/season.js') }}"></script>
@endpush
