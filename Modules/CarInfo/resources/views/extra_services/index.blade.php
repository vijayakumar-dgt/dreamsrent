...
<div class="page-wrapper">
    <div class="content me-4">
        <x-admin.breadcrumb :title="__('admin.rentals.extra_services')" :breadcrumbs="[
            __('admin.rentals.extra_services') => ''
        ]" :buttonText="__('admin.rentals.add_new_extra_service')" :modalId="'add_extra_service'"
            :buttonId="'add_new_extra_service'" :permissionModule="'extra_service'" />
        <!-- Table Header -->
            ...
            <div class="d-flex my-xl-auto right-content align-items-center flex-wrap row-gap-3">
                <div class="dropdown">
                    <button type="button" class="dropdown-toggle btn btn-white d-inline-flex align-items-center"
                        ...
                        <i class="ti ti-badge me-1"></i><span
                            class="status_label">{{ __('admin.common.status') }}</span>
                    </button>
                    <ul class="dropdown-menu  dropdown-menu-end p-2">
                        <li>
                            <button type="button" class="dropdown-item rounded-1 status_option" data-label="Active"
                                data-id="1">{{ __('admin.common.active') }}</button>
                        </li>
                        <li>
                            <button type="button" class="dropdown-item rounded-1 status_option" data-label="Inactive"
                                data-id="0">{{ __('admin.common.inactive') }}</button>
                        </li>
                    </ul>
                </div>
            </div>
                ...
                <table class="table" id="ExtraServiceTable">
                    <thead class="thead-light">
                        <tr>
                            ...
                            @if (hasPermission($permissions, 'extra_service', 'edit') || hasPermission($permissions,
...
<x-admin.modal className="addmodal" id="add_extra_service" :title="__('admin.rentals.create_extra_service')"
    action="{{  route('store_extra_service') }}" formId="extraServiceForm" method="POST" enctype="multipart/form-data">
    <x-slot name="body">
        <input type="hidden" name="id" id="id">
        <input type="hidden" name="language_id" id="language_id">
        <div class="mb-3">
            <label for="name" class="form-label">{{ __('admin.common.name') }} <span
        ...
        <div class="row mb-3">
            <label for="icon" class="form-label">{{ __('admin.common.icon') }} <span
                    class="text-danger icon_asterisk">*</span></label>
            <div class="col-md-4">
                <div
        ...
        <div class="d-flex justify-content-between align-items-center w-100">
            <div class="form-check form-check-md form-switch me-2 d-none" id="statusDiv">
                <label for="status" class="form-check-label form-label mt-0 mb-0">
                    <input class="form-check-input form-label me-2 status" name="status" id="status" type="checkbox"
