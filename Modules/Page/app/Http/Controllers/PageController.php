...
use Modules\Page\Services\PageBuilderService;
use Modules\Page\Services\PageSectionService;
...
class PageController extends Controller
{
    ...
    public function editPage(string $slug, Request $request): View
    {
        ...
        $slugsToTry = [$slug, Str::start($slug, PageConstants::PAGES)];
    ...
    public function getPageInfo(Request $request): JsonResponse
    {
        try {
            ...
            if (!$page) {
                $fallbackSlug = PageConstants::PAGES . ltrim($pageSlug, '/');
    ...
    public function pageStore(PageRequest $request): JsonResponse
    {
        ...
        // Proceed with storing
            ...
            $sections = $this->pageSectionService->prepareSections($request);
    ...
    {
        return $this->pageBuilderService->build($request, $slug);
    }
    ...
    public function getPage(string $slug): View
    {
        ...
        $fallbackSlug = PageConstants::PAGES . ltrim($slug, '/');
                    ...
                    $data = $vehicles->map(function ($vehicle) {
                        ...
                        if ($user) {
                            ...
                            $userProfileImg = $userDetail && $userDetail->profile_image
                                ? url(PageConstants::STORAGE_PATHS . $userDetail->profile_image)
                                : null;
                        }
                    ...
                    if (is_string($content) && strpos($content, '[blog') !== false) {
                        ...
                        foreach ($blogss as $blog) {
                            ...
                            $blogs[] = [
                                ...
                                'author'      => [
                                    'name'   => 'Admin',
                                    'avatar' => asset(PageConstants::DEFAULT_PROFILE),
                                ],
                            ];
                        }
