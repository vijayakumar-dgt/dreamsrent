...
use Modules\Page\Support\PageConstants;

class PageBuilderService
{
    public function build(Request $request, ?string $slug = null): View|JsonResponse
    {
        $themeId = $this->resolveThemeIdFromSlug($slug);
        $slug = $this->determinePageSlug($request, $slug, $themeId);

        if (!$slug) {
            return response()->json([
                'code'    => 404,
                'status'  => 'error',
                'message' => __('Slug must be specified'),
            ]);
        }

        $langId = getLanguageId(app()->getLocale());
        $page = $this->findPageForTheme($slug, $themeId, $langId);
        ...
        $pageContentSections = $this->preparePageSections($page, $langId, $themeId, $slug);

        return $this->preparePageBuilderResponse($request, $page, $pageContentSections, $themeId, $slug, $langId);
    }

    private function resolveThemeIdFromSlug(?string &$slug): int
    {
        if ($slug !== null && $slug !== '' && $slug !== '0') {
            $number = preg_match('/(\d+)$/', $slug, $matches) ? (int) ($matches[1] ?? null) : null;
        ...
        return $defaultThemeValue ? (int) $defaultThemeValue->value : 1;
    }

    private function determinePageSlug(Request $request, ?string $slug, int $themeId): ?string
    {
    ...
    {
        return $request->has('is_mobile') && $request->get('is_mobile') === 'yes';
    }

    private function findPageForTheme(string $slug, int $themeId, int $langId): ?Page
    {
        ...
        if (!$page) {
            ...
            if ($basePage) {
                $page = Page::where('parent_id', $basePage->id)
                    ->where('language_id', $langId)
                    ->where('theme_id', $themeId)
                    ->first();
            }
    ...
    private function preparePageSections(Page $page, int $langId, int $themeId, string $slug): array
    {
        $pageContentSections = json_decode($page->page_content ?? '[]', true) ?? [];

        if (empty($pageContentSections) || !collect((array) $pageContentSections)->contains(fn ($section) => ($section['status'] ?? 0) == 1)) {
            return [];
        }

        return $this->transformPageSections($pageContentSections, $langId, $themeId, $slug);
    }

    private function transformPageSections(array $pageContentSections, int $langId, int $themeId, string $slug): array
    {
        foreach ($pageContentSections as &$section) {
            ...
            if (is_array($section) && ($section['status'] ?? 0) == 1) {
                ...
                if (is_string($content) && strpos($content, '[banner_one') !== false) {
                    preg_match(PageConstants::LIMIT_VIEWALL_ORDER_REGEX, $content, $matches);

                    $limit = (int)($matches[1] ?? 10);
                    $order = $matches[3] ?? 'asc';

                    $banners = DB::table('sections')
                        ->join('section_datas', function ($join) use ($langId) {
                            $join->on('sections.id', '=', 'section_datas.section_id')
                                ->where('section_datas.language_id', '=', $langId);
                        })
                    ...
                    foreach ($banners as &$banner) {
                        ...
                        $relativePath = PageConstants::STORAGE_PATH . ($decodedData['thumbnail_image_one'] ?? '');
                        $defaultImage = asset(PageConstants::PLACEHOLDER_BANNER);
            ...
            if (($section['status'] == 1) && (isset($section['section_content']) && strpos($section['section_content'], '[banner_two') !== false)) {
                preg_match(PageConstants::LIMIT_VIEWALL_ORDER_REGEX, $section['section_content'], $matches);
                ...
                foreach ($banners as &$banner) {
                    ...
                    $relativePath = PageConstants::STORAGE_PATH . ($decodedData['thumbnail_image_two'] ?? '');
                    $defaultImage = asset(PageConstants::PLACEHOLDER_BANNER);
                    ...
                    $banner->customer_images = [
                        asset(PageConstants::DEFAULT_AVATAR_01),
                        asset(PageConstants::DEFAULT_AVATAR_02),
                        asset(PageConstants::DEFAULT_AVATAR_03),
                    ];
            ...
            if (($section['status'] == 1) && (isset($section['section_content']) && strpos($section['section_content'], '[banner_three') !== false)) {
                ...
                foreach ($banners as &$banner) {
                    ...
                    $relativePath = PageConstants::STORAGE_PATH . ($decodedData['thumbnail_image_four'] ?? '');
                    $defaultImage = asset(PageConstants::PLACEHOLDER_BANNER);
                ...
                foreach ($banners as &$banner) {
                    ...
                    if (!empty($decodedData['thumbnail_image_boat']) && is_array($decodedData['thumbnail_image_boat'])) {
                        foreach ($decodedData['thumbnail_image_boat'] as $imagePath) {
                            $fullPath = public_path(PageConstants::STORAGE_PATH . $imagePath);
                            $thumbnailImages[] = file_exists($fullPath) ? asset(PageConstants::STORAGE_PATH . $imagePath) : $defaultImage;
                        }
                    }
            ...
            if (is_array($section) && ($section['status'] ?? 0) == 1) {
                ...
                if (is_string($content) && strpos($content, '[bestVehicle') !== false) {
                    ...
                    foreach ($best_vehicles as &$best_vehicle) {
                        ...
                        if ($vehicleId) {
                            $vehicle = VehicleInfo::where("language_id", $langId)->find($vehicleId);

                            $best_vehicle->vehicle_name = $vehicle->name ?? null;

                            $imagePath = $vehicle->vehicle_image ?? null;
                            $best_vehicle->vehicle_image_url = $imagePath ? asset(PageConstants::STORAGE_PATH . $imagePath) : null;
                        } else {
            ...
            if (is_array($section) && ($section['status'] ?? 0) == 1) {
                ...
                if (is_string($content) && strpos($content, '[brand') !== false) {
                    ...
                    $brands = DB::table('brands')
                        ...
                        ->get()
                        ->map(function ($brand) {
                            $brand->brand_image = asset(PageConstants::STORAGE_PATH . $brand->brand_image);
                            $brand->brand_icon = asset(PageConstants::STORAGE_PATH . $brand->brand_icon);
                            return $brand;
                        });
            ...
            if (is_array($section) && ($section['status'] ?? 0) == 1) {
                ...
                if (is_string($content) && strpos($content, '[location') !== false) {
                    ...
                    asset(PageConstants::PLACEHOLDER_APP_CAR);
                    ...
                    $locations = DB::table('locations')
                        ...
                        ->map(function ($location) use ($defaultImage) {
                            if ($location->image && Storage::disk('public')->exists($location->image)) {
                                $location->image = asset(PageConstants::STORAGE_PATH . $location->image);
                            } else {
            ...
            if ((is_array($section) && ($section['status'] ?? 0) == 1) && (
                ...
                $category = Cartype::select('name', 'icon', 'id')
                    ...
                    ->orderBy('created_at', $order)
                    ->get()
                    ->map(function ($cartype) use ($langId) {
                        $cartype->car_count = VehicleInfo::where('type_id', $cartype->id)
                            ->where('language_id', $langId)
                            ->count();

                        $cartype->image_url = $cartype->icon
                            ? asset(PageConstants::STORAGE_PATH . ltrim($cartype->icon, '/'))
                            : asset(PageConstants::DEFAULT_IMAGE);
                ...
                $category = Cartype::select('name', 'icon', 'id', 'type')
                    ...
                    ->map(function ($cartype) use ($langId) {
                        $cartype->boat_count = VehicleInfo::where('type_id', $cartype->id)
                            ->where('language_id', $langId)->where('type', 'boat')
                            ->count();
            ...
            if (is_array($section) && ($section['status'] ?? 0) == 1) {
                ...
                if (is_string($content) && strpos($content, '[facts_with_faq') !== false) {
                    ...
                    $locationCount = Location::where('language_id', $langId)->count();
                    $totalKm = 1976;
            ...
            // FAQ Section (with Facts and Blogs)
            if (is_array($section) && ($section['status'] ?? 0) == 1) {
                $content = $section['section_content'] ?? '';
            ...
            if (($section['status'] == 1) && (isset($section['section_content']) && strpos($section['section_content'], '[vehicle') !== false)) {
                preg_match(PageConstants::TYPE_LIMIT_VIEWALL_REGEX, $section['section_content'], $matches);
                $type = $matches[1] ?? 'all';

                $query = VehicleInfo::with([
                    PageConstants::CAR_TYPE_SELECT,
                    PageConstants::BRAND_SELECT,
                    PageConstants::CATEGORY_SELECT,
                    PageConstants::MAIN_LOCATION_SELECT,
                    PageConstants::COLOR_SELECT,
                    PageConstants::FUEL_TYPE_SELECT,
                    PageConstants::TRANSMISSION_SELECT,
                ])->where('language_id', $langId);
                ...
                $data = $vehicles->map(function ($vehicle) {
                    ...
                    $multipleImages = array_map(function ($img) {
                        ...
                        $img = str_replace(PageConstants::VEHICLE_IMAGE, PageConstants::VEHICLE_IMAGE_SMALL, $img);
                    ...
                    $defaultAvatar = asset(PageConstants::DEFAULT_PROFILE);
                    $profileImagePath = optional($vehicle->owner->userDetails)->profile_image;

                    $avatarImage = $defaultAvatar;

                    if ($profileImagePath) {
                        $fullImagePath = storage_path(PageConstants::APP_PUBLIC_PATH . $profileImagePath);
                        if (file_exists($fullImagePath)) {
                            $avatarImage = url(PageConstants::STORAGE_PATHS . $profileImagePath);
                        }
                    }
                    return [
                        'id'                      => $vehicle->id,
                        'name'                    => $vehicle->name,
                        'slug'                    => $vehicle->slug,
                        'vehicle_image'           => url(PageConstants::STORAGE_PATHS . str_replace(PageConstants::VEHICLE_IMAGE, PageConstants::VEHICLE_IMAGE_SMALL, $vehicle->vehicle_image)),
            ...
            if (is_array($section) && ($section['status'] ?? 0) == 1) {
                ...
                if (is_string($content) && strpos($content, '[car_type ') !== false) {
                    preg_match(PageConstants::LIMIT_VIEWALL_REGEX, $content, $matches);
                ...
                if (is_string($content) && strpos($content, '[ad_card') !== false) {
                    ...
                    $how_it_works = DB::table('general_settings')
                        ...
                        ->map(function ($item) {
                            ...
                            $value['image'] = asset(PageConstants::PLACEHOLDER_APP_CAR);
                            ...
                            foreach ($data as $key => $val) {
                                if (str_starts_with($key, 'thumbnail_image_') && !empty($val)) {
                                    $imageUrl = Storage::disk('public')->exists(ltrim($val, '/'))
                                        ? asset(PageConstants::STORAGE_PATH . ltrim($val, '/'))
                                        : asset(PageConstants::PLACEHOLDER_APP_CAR);
                                    $imageList[] = $imageUrl;
                                }
                            }
                ...
                if (is_string($content) && strpos($content, '[why_us') !== false) {
                    ...
                    if ($whyus->isNotEmpty()) {
                        ...
                        foreach ([1, 2, 3] as $i) {
                            ...
                            if (empty($icon)) {
                                if ($i === 1) {
                                    $icon = asset(PageConstants::ICON_SELECTION);
                                } elseif ($i === 2) {
                                ...
                                $icon = asset(PageConstants::STORAGE_PATH . $icon);
                            }
                ...
                if (is_string($content) && strpos($content, '[exclusive_bike') !== false) {
                    ...
                    if ($whyus->isNotEmpty()) {
                        ...
                        if (!empty($data['thumbnail_image_bike_exclusive'])) {
                            ...
                            $fullPath = storage_path(PageConstants::APP_PUBLIC_PATH . $relativePath);
                            if (file_exists($fullPath)) {
                                $previewImage = asset(PageConstants::STORAGE_PATH . $relativePath);
                            }
                        }
                            ...
                            if (is_array($data)) {
                                ...
                                foreach ($data as $key => $value) {
                                    if (str_starts_with($key, 'thumbnail_image_')) {
                                        ...
                                        $fullPath = storage_path(PageConstants::APP_PUBLIC_PATH . $imagePath);

                                        if (!empty($value) && file_exists($fullPath)) {
                                            $data[$key] = asset(PageConstants::STORAGE_PATH . $imagePath);
                                        } else {
            ...
            if (is_array($section) && ($section['status'] ?? 0) == 1) {
                ...
                if (is_string($content) && strpos($content, '[yart_benefit') !== false) {
                    ...
                    if ($benefits->isNotEmpty()) {
                        ...
                        $items = [];
                        $fallbacks = [
                            1 => PageConstants::ICON_SELECTION,
                            2 => '/frontend/assets/img/icons/bx-crown.svg',
                            3 => '/frontend/assets/img/icons/bx-user-check.svg',
                            4 => PageConstants::ICON_SELECTION,
                            5 => PageConstants::ICON_SELECTION,
                        ...
                        foreach (range(1, 6) as $i) {
                            ...
                            if (!empty($thumbnail)) {
                                ...
                                if (file_exists($fullPath)) {
                                    $image = asset(PageConstants::STORAGE_PATH . $relativePath);
                                }
                            }
                        ...
                        if (!empty($data['thumbnail_image_boat_benefits_main'])) {
                            ...
                            if (file_exists($fullPath)) {
                                $mainImage = asset(PageConstants::STORAGE_PATH . $relativePath);
                            }
                        }
            ...
            if (is_array($section) && ($section['status'] ?? 0) == 1) {
                ...
                if (is_string($content) && strpos($content, '[blog') !== false) {
                    preg_match(PageConstants::TYPE_LIMIT_VIEWALL_REGEX, $content, $matches);
                    $type = $matches[1] ?? 'all';
                    $limit = $matches[2] ?? 10;
            ...
            if (is_array($section) && ($section['status'] ?? 0) == 1) {
                ...
                if (is_string($content) && strpos($content, '[search') !== false) {
                    ...
                    $section['section_content'] = [
                        ...
                        "description" => PageConstants::FIND_VEHICLES_SERVICE
                    ];
                }
            }
            ...
            if (is_array($section) && ($section['status'] ?? 0) == 1) {
                ...
                if (is_string($content) && strpos($content, '[marquee') !== false) {
                    ...
                    $page = Page::select("keywords")->where("slug", $slug)->where("language_id", $langId)->first();

                    if (!$page) {
                        $basePage = Page::select("keywords", "id")->where('slug', $slug)->whereNull('parent_id')->first();

                        if ($basePage) {
                            $page = Page::select("keywords")->where('parent_id', $basePage->id)->where('language_id', $langId)->first();
                        }
                    }
                                ...
                                foreach ($data as $key => $value) {
                                    if (str_starts_with($key, 'thumbnail_image_') && !empty($value)) {
                                        ...
                                        if (Storage::disk('public')->exists(ltrim($value, '/'))) {
                                            $data[$key] = asset(PageConstants::STORAGE_PATH . ltrim($value, '/'));
                                        } else {
                                            // Fallback default image path
                                            $data[$key] = asset(PageConstants::PLACEHOLDER_APP_CAR);
                                        }
                                    }
                                }
                    ...
                    if ($experiences->isNotEmpty()) {
                        ...
                        $data = $vehicles->map(function ($vehicle) {
                            ...
                            return [
                                ...
                                'name'                    => $vehicle->name,
                                'slug'                    => $vehicle->slug,
                                'vehicle_image'           => url(PageConstants::STORAGE_PATHS . $vehicle->vehicle_image),
                                'multiple_vehicle_images' => $multipleImages,
                                'has_multiple_image'      => count($multipleImages) > 1,
                                'avatar_image'            => $avatarImage,
                                'brand_id'                => $vehicle->brand_id ?? null,
        ...
        return $pageContentSections;
    }

    private function preparePageBuilderResponse(Request $request, Page $page, array $pageContentSections, int $themeId, string $slug, int $langId): View|JsonResponse
    {
        ...
        $page = Page::where('slug', $slug)->where('theme_id', $themeId)->where('language_id', $langId)->first();
        if (!$page) {
            $basePage = Page::where('slug', $slug)->whereNull('parent_id')->first();

            if ($basePage) {
                $page = Page::where('parent_id', $basePage->id)->where('language_id', $langId)->where('theme_id', $themeId)->first();
            }
        }
        if ($page) {
            $data = [
            ...
            if ($this->isMobileRequest($request)) {
                ...
                $returnResponse = view($viewPath, compact('data', 'content_sections', 'vehicleBrand', 'seo_title', 'seo_description', 'og_title', 'og_description', 'meta_keywords'));
            }
