(async function () {
    ...
    $(document).ready(function () {
        getLocations("", function (response) {
            appendLocationData(
                "#pickUpLocationList .custom-scroll",
                "pickup_location_checkbox",
                response
            );
            appendLocationData(
                "#dropOffLocationList .custom-scroll",
                "drop_location_checkbox",
                response
            );
        });
        bookingList();
    ...
    function bookingList(sort_by_date = "") {
        $("#reservationTable").DataTable({
            ...
            ajax: {
                ...
                type: "POST",
                data: function (d) {
                    d.pickup_location_ids = $(
                        ".pickup_location_checkbox:checked"
                    )
                        .map(function () {
                            return $(this).val();
                        })
                        .get();
                    d.drop_location_ids = $(".drop_location_checkbox:checked")
                        .map(function () {
                            return $(this).val();
                        })
                        .get();
                    d.status = $(".status_checkbox:checked")
                        .map(function () {
                            return $(this).val();
                        })
                        .get();
                ...
                complete: function () {
                    ...
                    if (
                        $("#reservationTable").DataTable().rows().count() == 0
                    ) {
            ...
            columns: [
                {
                    ...
                    render: function (data, type, row) {
                        return `<div class="d-flex align-items-center">
                            ...
                            <div>
                                <a href="/admin/quotations-details/${
                                    row.encrypted_id
                                }" class="text-info d-block mb-1">#${
                            row.reservation_id
                        }</a>
                                <h6 class="fs-14 text-black">${
                                    row.vehicle_name
                                }</h6>
                            </div>
                        </div>
                    ...
                    render: function (data, type, row) {
                        return `<div class="d-flex align-items-center">
                                <div class="avatar avatar-rounded me-2 flex-shrink-0">
                                    <img src="${row.customer_image}" alt="${_l(
                            "admin.common.image"
                        )}">
                                </div>
                                <div>
                                    <h6 class="mb-1 fs-14 text-black">${
                                        row.customer_full_name
                                            ? row.customer_full_name
                                            : ""
                                    }</h6>
                                    <span class="badge bg-secondary-transparent rounded-pill">${_l(
                                        "admin.common.client"
                                    )}</span>
                                </div>
                            </div>
                    ...
                    render: function (data, type, row) {
                        ...
                        <div class="dropdown">
                            ...
                            <ul class="dropdown-menu dropdown-menu-end p-2">
                                <li>
                                    <a class="dropdown-item rounded-1" href="/admin/quotations-details/${
                                        row.encrypted_id
                                    }">
                                        <i class="ti ti-eye me-1"></i>${_l(
                                            "admin.common.view_details"
                                        )}
                                    </a>
                                </li>
                                ${
                                    row.booking_status != 6 &&
                                    row.booking_by != "user" &&
                                    hasPermission(
                                        permissions,
                                        "quotations",
                                        ...
                                        ? `
                                    <li>
                                        <a class="dropdown-item rounded-1" href="/admin/edit-quotations/${
                                            row.encrypted_id
                                        }">
                                ...
                                ${
                                    ...
                                    <li>
                                        <a class="dropdown-item rounded-1 ${
                                            isTodayOrFuture ? "" : "d-none"
                                        }" href="/admin/edit-reservation/${
                                              row.encrypted_id
                                          }">
                                            <i class="ti ti-calendar me-1"></i>${_l(
                                                "admin.common.covert"
                                            )}
                                        </a>
                                    </li>
                                ...
                                ${
                                    hasPermission(
                                        ...
                                        ? `
                                    <li>
                                        <button type="button" class="dropdown-item rounded-1 delete-quotation" data-id="${
                                            row.id
