(async () => {
            ...
            success: function (result) {
                if (result.data && result.data.data.length > 0) {
                    ...
                    let options = data
                        .map(function (item) {
                            let is_checked =
                                item.id == edit_vehicle_id ? "checked" : "";
                                            ...
                                            <div class="d-flex align-items-center">
                                                <div class="form-check form-check-md me-3">
                                                    <input class="form-check-input vehicle_select" name="vehicle_id" id="vehicle_select_${
                                                        item.id
                                                    }" value="${item.id}" type="radio" data-price="${item.vehicle_price}" data-price_type="${item.vehicle_price_type}" data-vehicle_name="${item.vehicle_name}" data-vehicle_season_id="${item.vehicle_season_id ?? ""}" data-vehicle_tariff_id="${item.vehicle_tariff_id ?? ""}" ${is_checked}>
                                                </div>
            ...
            success: function (result) {
                if (result.data && result.data.length > 0) {
                    ...
                    let options = data
                        .map((item) => {
                            let isChecked =
                                selected_extra_service_ids.includes(
                                    item.id.toString()
                                ) ||
                                edit_extra_service.find(
            ...
            success: function (result) {
                if (result.data && result.data.length > 0) {
                    ...
                    let options = data
                        .map((item) => {
                            let isChecked =
                                selected_insurance_ids.includes(
                                    item.id.toString()
                                ) ||
                                edit_insurance.find(
                                    (service) => service.id == item.id
    ...
    function calculateVehiclePrice() {
        ...
        var driverPriceVal =
            parseFloat($("#driver_price").val()).toFixed(2) || 0;
        var securityDeposit =
                    ...
                    $("#driver_details_list").html(`
                        <div class="d-flex align-items-center justify-content-end mb-3">
                            <button type="button" class="text-purple text-decoration-underline fw-medium edit_driver_price border-0 bg-transparent" data-bs-toggle="modal" data-bs-target="#edit_price_modal"
                                ...
                                data-phone="${data.phone_number}" data-price="${
                        edit_driver_price != "" ? edit_driver_price : 0
                    }">${_l("admin.bookings.edit_price")}
                            </button>
                        </div>
                                        ...
                                        <div class="row gx-2 gy-3">
                                            ...
                                            <div class="col-md-3">
                                                <div>
                                                    ...
                                                    <p>${default_currency}<span class="td-driver-price">${
                        edit_driver_price != "" ? edit_driver_price : 0
                    }</span></p>
                                                </div>
                                            </div>
