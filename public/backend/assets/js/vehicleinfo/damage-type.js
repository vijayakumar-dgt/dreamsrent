(async () => {
            ...
            success: function (response) {
                ...
                if (response.code === 200 && response.data.length > 0) {
                    ...
                    $.each(data, function (index, value) {
                        tableBody += `
                                <tr>
                                    <td class="text-start"><h6 class="fw-medium text-black">${
                                        value.damage_type
                                    }</h6></td>
                                    <td class="text-start">
                                        <span class="badge ${
    ...
    function initFormValidation() {
        $("#damageTypeForm").validate({
            ...
            submitHandler: function (form) {
                ...
                $("#add_damage_type .submitbtn").html(
                ...
                $.ajax({
                    ...
                    success: function (resp) {
                        ...
                        $("#add_damage_type .submitbtn").text(
                            _l("admin.common.create_new")
                        );
                        $("#add_damage_type .submitbtn").attr(
                            "disabled",
                            false
                        );
                    },
    ...
    function initEvents() {
        $(document).on("click", "#add_new_damage_type", function () {
            $("#add_damage_type .modal-title").text(
                _l("admin.rentals.create_damage_type")
            );
            $("#add_damage_type .submitbtn").text(
                _l("admin.common.create_new")
            );
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.code === 200) {
                    ...
                    $("#add_damage_type #status").prop(
                        "checked",
                        data.status === 1
                    );

                    $("#add_damage_type .modal-title").text(
                        _l("admin.rentals.edit_damage_type")
                    );
                    $("#add_damage_type .submitbtn").text(
                    ...
                    showToast(
                        "error",
                        response.message || _l("admin.common.fetch_error")
                    );
                }
            },
            error: function () {
