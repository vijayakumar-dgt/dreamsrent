(async () => {
        ...
        $.validator.addMethod(
            ...
            function (value, element, param) {
                return (
                    ...
                    element.files[0].size <= param * 2048
                );
            },
            ...
            success: (response) => {
                if (response.code === 200) {
                    populateDropdown(
                        "#country",
                        response.data,
                        _l("admin.common.select_country")
                    );
                }
            },
        });
            ...
            success: (response) => {
                if (response.code === 200) {
                    populateDropdown(
                        "#state",
                        response.data,
                        _l("admin.common.select_state")
                    );
                    $("#city").html(
                        `<option value="">${_l(
                            "admin.common.select_city"
                        )}</option>`
                    );
                }
            },
        ...
        $.ajax({
            ...
            success: (response) => {
                if (response.code === 200) {
                    ...
                    $addLocationModal
                        .find(".modal-title")
                        .text(_l("admin.manage.edit_location"));
                    $addLocationModal
                        .find(".submitbtn")
    ...
    const setupEventHandlers = () => {
        ...
        $(document).on("click", "#add_new_location", function () {
            $addLocationModal
                .find(".modal-title")
                .text(_l("admin.manage.add_location"));
            $addLocationModal
                .find(".submitbtn")
    ...
    const resetLocationDropdowns = () => {
        ...
        $("#city").html(
            `<option value="">${_l("admin.common.select_city")}</option>`
        );
    };
})();
