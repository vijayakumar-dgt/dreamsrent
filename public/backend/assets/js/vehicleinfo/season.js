(async () => {
        ...
        $("#seasonForm").validate({
            ...
            submitHandler: function (form) {
                ...
                $.ajax({
                    ...
                    processData: false,
                    contentType: false,
                    beforeSend: handleSeasonBeforeSend,
                    complete: handleSeasonComplete,
                    success: handleSeasonSuccess,
    ...
    function handleSeasonBeforeSend() {
        $(".submitbtn")
            .attr("disabled", true)
            .html(`
    ...
    function handleSeasonSuccess(resp) {
        if (resp.code === 200) {
    ...
    function handleSeasonError(error) {
        resetSeasonValidation();

        const response = error.responseJSON;
        if (!response) return showToast("error", "Unexpected error occurred.");

        if (response.code === 422) {
            displaySeasonValidationErrors(response.errors);
        } else {
            showToast("error", response.message);
        }
    }

    function resetSeasonValidation() {
        $(".error-text").text("");
        $(".form-control").removeClass("is-invalid is-valid");
    }

    function displaySeasonValidationErrors(errors) {
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-end p-2">
                                            ${
                                                hasPermission(
                                                    ...
                                                    ? `<li>
                                                <button type="button" class="dropdown-item rounded-1 editSeason" data-id="${
                                                    value.id
                                                }">
                                            ...
                                            ${
                                                hasPermission(
                                                    ...
                                                    ? `<li>
                                                <button type="button" class="dropdown-item rounded-1 deleteSeason" data-id="${
                                                    value.id
    ...
    function initEvents() {
        ...
        $(document).on("click", "#add_new_season", function () {
            $("#add_season .modal-title").text(
                _l("admin.rentals.create_season")
            );
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.code === 200) {
                        ...
                        $("#add_season .modal-title").text(
                            _l("admin.rentals.edit_season")
                        );
                        $("#add_season .submitbtn").text(
                            _l("admin.common.save_changes")
                        );
