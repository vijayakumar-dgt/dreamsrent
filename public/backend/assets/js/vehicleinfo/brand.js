(async () => {
    ...
    function initValidation() {
        ...
        $.validator.addMethod("filesize", (value, element, param) => {
            return element.files.length === 0 || element.files[0].size <= param * 1024;
        }, "File size must be less than {0} KB.");

        $.validator.addMethod("iconDimension", validateImageDimension(25, 25), _l('admin.rentals.brand_icon_dimension'));
        $.validator.addMethod("imageDimension", validateImageDimension(180, 180), _l('admin.common.image_pixel'));
    }

    function validateImageDimension(minWidth, minHeight) {
        ...
        $.ajax({
            ...
            beforeSend: () => {
                $('.submitbtn').attr('disabled', true).html(`
                    <span class="spinner-border spinner-border-sm align-middle" role="status"></span> ${_l('admin.common.saving')}..
                `);
            },
            success: function (resp) {
                $('.submitbtn').prop('disabled', false).html($('#id').val() ? _l('admin.common.save_changes') : _l('admin.common.create_new'));
                if (resp.code === 200) {
            ...
            error: function (err) {
                ...
                if (err.responseJSON?.code === 422) {
                    ...
                    showToast('error', err.responseJSON?.message || "Unknown error occurred.");
                }
            }
        });
    }
    ...
    function initEvents() {
        $('#brand_image, #brand_icon').on('change', function () {
            const input = this;
            const previewId = input.id === "brand_image" ? "#imagePreview" : "#iconPreview";
            const iconClass = input.id === "brand_image" ? ".upload_icon" : ".upload_icon_2";

            $(input).valid();
            if (input.files && input.files[0]) {
                const reader = new FileReader();
                reader.onload = e => $(previewId).attr('src', e.target.result).removeClass('d-none');
        ...
        $('#search').on('keyup', () => $('#brandTable').DataTable().ajax.reload());

        $(document).on('click', '#status_filter .dropdown-item', function () {
            const sortBy = $(this).data('status');
            $('#sort_by_status').val(sortBy);
            $('#current_sort_status').text(sortBy == 1 ? _l('admin.common.active') : _l('admin.common.inactive'));
        ...
        $('#add_brand').on('click', () => {
            ...
            $(".form-control").removeClass("is-invalid is-valid");
            $(".error-text").text("");
            $('#statusDiv').addClass('d-none').parent().addClass('justify-content-end').removeClass('justify-content-between');
        ...
        $("#brandDeleteForm").on('submit', function (e) {
            ...
            $.ajax({
                ...
                error: function (res) {
                    const msg = res.responseJSON?.message || _l('admin.common.default_delete_error');
                    showToast(res.responseJSON?.code === 500 ? 'success' : 'error', msg);
                }
            });
        });
