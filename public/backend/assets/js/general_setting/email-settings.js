...
$(document).ready(function() {
    ...
    $("#php_mailer_form").validate({
        ...
        messages:{
            phpmail_from_email: {
                required: _l('admin.general_settings.from_email_address_required'),
                email: _l('admin.general_settings.from_email_address_valid'),
            },
            phpmail_password: {
                required: _l('admin.general_settings.email_password_required'),
            },
            phpmail_from_name: {
                required: _l('admin.general_settings.from_email_name_required'),
            },
        },
        ...
        submitHandler: function(form) {
            ...
            $.ajax({
                ...
                success:function(resp){
                    ...
                    $(".submitBtn").removeAttr("disabled").html($('.submitBtn').data('submit'));
                    if (resp.code === 200) {
    ...
    $("#sendgrid_form").validate({
        ...
        messages:{
            ...
            sendgrid_key: {
                required: _l('admin.general_settings.send_grid_key_required'),
            },
        },
        errorPlacement: function (error, element) {
            if (element.hasClass("select2-hidden-accessible")) {
    ...
    $("#test_mail_form").validate({
        ...
        messages:{
            email_address: {
                required: _l('admin.general_settings.email_address_required'),
...
$(document).on('change', '.status-switch', function() {
    ...
    $(this).closest('.card').find('.status-text').html(`<i class="ti ti-point-filled text-success"></i> ${_l('admin.general_settings.connected')}`);
...
function loadEmailSettings() {
    $.ajax({
                ...
                filteredSettings.forEach((setting) => {
                    if (setting.key === "phpmail_status") {
                        if (setting.value === "1") {
                            ...
                            $('.phpmail-status-text').html(`<i class="ti ti-point-filled text-success"></i> ${_l('admin.general_settings.connected')}`);
                        } else {
                            $("#phpmail_status").prop("checked", false);
                            $('.phpmail-status-text').html(`<i class="ti ti-point-filled text-danger"></i> ${_l('admin.general_settings.disconnected')}`);
                        }
                    } else if (setting.key === "smtp_status") {
                        if (setting.value === "1") {
                            $("#smtp_status").prop("checked", true);
                            $('.smtp-status-text').html(`<i class="ti ti-point-filled text-success"></i> ${_l('admin.general_settings.connected')}`);
                        } else {
                            $("#smtp_status").prop("checked", false);
                            $('.smtp-status-text').html(`<i class="ti ti-point-filled text-danger"></i> ${_l('admin.general_settings.disconnected')}`);
                        }
                    } else if (setting.key === "sendgrid_status") {
                        if (setting.value === "1") {
                            $("#sendgrid_status").prop("checked", true);
                            $('.sendgrid-status-text').html(`<i class="ti ti-point-filled text-success"></i> ${_l('admin.general_settings.connected')}`);
                        } else {
                            $("#sendgrid_status").prop("checked", false);
                            $('.sendgrid-status-text').html(`<i class="ti ti-point-filled text-danger"></i> ${_l('admin.general_settings.disconnected')}`);
                        }
                    } else {
        ...
        complete: function () {
            $(".label-loader, .input-loader, .card-loader, .button-loader").hide();
            $('.real-label, .real-input, .real-button, .real-card').removeClass('d-none');
        }
    });
}
