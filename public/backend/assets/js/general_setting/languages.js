(async () => {
    ...
    $(document).ready(function(){
        loadLanguages();
        $(document).on('click','#langDropdownMenu li .dropdown-item', function(){
              let lang_title = $(this).data('lang_title') ?? 'Language';
              $("#langDropdownMenu li .dropdown-item").removeClass('selected');
        ...
        $(document).on('click','#addNewLanguage', function(){
            ...
            if(lang_id){
                $.ajax({
                    ...
                    data:{lang_id:lang_id, lang_title:lang_title, _token:$('meta[name="csrf-token"]').attr('content')},
                    beforeSend: function() {
                        $("#addNewLanguage").html('<span class="spinner-border spinner-border-sm align-middle" role="status" aria-hidden="true"></span> Adding..');
        ...
        $(document).on('change','.form-check-input',function(){
            if($(this).is(':checked')){
                ...
                updateLanguageSettings($(this).data('id'),$(this).data('field'),0);
            }
        });
        function updateLanguageSettings(id,field,value){
             $.ajax({
                 ...
                 data:{id:id,field:field,value:value,_token:$('meta[name="csrf-token"]').attr('content')},
                 success: function(response) {
                     if(response.code === 200){
                         showToast('success', response.message);
                     }else{
