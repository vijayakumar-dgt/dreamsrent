(async () => {
  ...
  $(document).ready(function () {
    ...
    $("#bankForm").validate({
      ...
      messages: {
        bank_name: { required: _l('admin.general_settings.bank_required') },
        account_number: {
        ...
        branch: { required: _l('admin.general_settings.branch_field_required') },
        ifsc: {
    ...
    $("#delateBank").on("submit", function (e) {
      ...
      $.ajax({
        ...
        error: function (res) {
          const msg = res.responseJSON?.message || _l('admin.general_settings.retrieve_error');
          showToast("error", msg);
        }
      });
    });
  ...
  function initTable(search = "") {
    $.ajax({
      ...
      success: function (response) {
        ...
        if (response.code === 200 && response.data.length > 0) {
          response.data.forEach((value) => {
            const statusBadge = value.default == 1
              ? "badge-success-transparent"
              : "badge-danger-transparent";

            tableBody += `<tr>
              <td>${value.bank_name.length > 24 ? value.bank_name.substring(0, 24) + "..." : value.bank_name}</td>
              <td>${value.account_number}</td>
              <td>${value.account_holder_name}</td>
              <td>${value.branch}</td>
              <td>${value.ifsc}</td>
              <td><span class="badge ${statusBadge} d-inline-flex align-items-center badge-sm">
              <i class="ti ti-point-filled me-1"></i>${value.default == 1 ? _l('admin.common.active') : _l('admin.common.inactive')}</span></td>
              ...
              <td>
                <div class="dropdown">
                  ...
                  <ul class="dropdown-menu dropdown-menu-end p-2">
                    ${hasPermission(permissions, 'finance_settings', 'edit') ? `<li><button class="dropdown-item rounded-1" data-id="${value.id}" id="edit-bank"><i class="ti ti-edit me-1"></i>${_l('admin.common.edit')}</button></li>` : ''}
                    ${hasPermission(permissions, 'finance_settings', 'delete') ? `<li><button class="dropdown-item rounded-1" data-id="${value.id}" id="delete-bank" data-bs-toggle="modal" data-bs-target="#delete_bank"><i class="ti ti-trash me-1"></i>${_l('admin.common.delete')}</button></li>` : ''}
                  </ul>
                </div>
              </td>` : ''}
            </tr>`;
          ...
          tableBody += `<tr><td colspan="4" class="text-center">${_l('admin.general_settings.no_data')}</td></tr>`;
        }
      ...
      error: function (error) {
        showToast("error", error.responseJSON?.message || _l('admin.general_settings.retrieve_error'));
      }
    });
  }
