...
function getVehicles(filterData = {}, isLoadMore = false) {
    ...
    $.ajax({
                                    ...
                                    <div class="col-3">
                                        <div class="d-flex align-items-center">
                                            <div class="form-check form-check-md me-3">
                                                <input class="form-check-input vehicle_select" name="vehicle_id" id="vehicle_select_${item.id}" value="${item.id}" type="radio" data-price="${item.vehicle_price}" data-price_type="${item.vehicle_price_type}" data-vehicle_name="${item.vehicle_name}" data-vehicle_season_id="${item.vehicle_season_id ?? ''}" data-vehicle_tariff_id="${item.vehicle_tariff_id ?? ''}">
                                            </div>
        ...
        complete: function () {
            ...
            $('#vehicle_list_container, .real-label, .real-input').removeClass('d-none');
        }
    });
}
            ...
            if (response.code === 200 && response.data) {
                ...
                $('#driver_details_list').html(`
                    <div class="d-flex align-items-center justify-content-end mb-3">
                        ...
                        data-phone="${data.phone_number}" data-price="0">${_l('admin.bookings.edit_price')}</a>
                    </div>
...
function calculateVehiclePrice() {
    ...
    var securityDeposit = $('#security_deposit').val() !== '' ? parseFloat($('#security_deposit').val()) : 0;
    var response = [];
        ...
        $('#extra_service_summary').append(`
            <div class="border-bottom mb-3 border-top pt-3">
                ...
                <div class="d-flex align-items-center justify-content-between ${typeof driver_name !== 'undefined' && driver_name !== '' ? '' : 'd-none'}">
                    <div class="mb-3">
                        <h6 class="d-inline-flex align-items-center fs-14 fw-medium ">${_l('admin.common.driver')}<a href="#" class="ms-2 d-none"><i class="ti ti-edit"></i></a></h6>
                    </div>
                    <div class="d-flex align-items-center mb-3">
                        <span class="avatar avatar-rounded flex-shrink-0 me-2">
            ...
            <div class="border-bottom mb-3 pb-2" id="price_container">
                <div class="d-flex align-items-center justify-content-between mb-2">
                    <h6 class="fw-medium fs-14">${_l('admin.bookings.pricing_of_vehicle')}</h6>
    ...
    $.ajax({
        ...
        success: function (result) {
            if (result.data && result.data.length > 0) {
                ...
                let options = data.map(item => {
                    let isChecked = selected_extra_service_ids.includes(item.id.toString()) ? 'checked' : '';
                    let isActive = selected_extra_service_ids.includes(item.id.toString()) ? 'active' : '';
    ...
    $.ajax({
        ...
        success: function (result) {
            if (result.data && result.data.length > 0) {
                ...
                let options = data.map(item => {
                    let isChecked = selected_insurance_ids.includes(item.id.toString()) ? 'checked' : '';
                    let isActive = selected_insurance_ids.includes(item.id.toString()) ? 'active' : '';
