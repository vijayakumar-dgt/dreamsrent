(async () => {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.success) {
                    ...
                    showToast("error", "No seasonal data found.");
                }
            },
            error: function (xhr, status, error) {
    ...
    $(document).ready(function () {
        ...
        // Fetch data when filters or name input change
        ...
        // Trigger API call when typing in the name input
        $("#name").on("keyup", function () {
            fetchFilteredData();
        });

        // Trigger API call when select sort by input
        ...
        // Trigger API call when Apply button is clicked
                        ...
                        tableBody += `<tr>
                            ...
                            <div class="d-flex align-items-start">
								<a href="car-details.html" class="avatar me-2 flex-shrink-0">
									<img src="/assets/img/car/car-01.jpg" class="rounded-3" alt="">
								</a>
								<div class="text-start">
									<h6><a href="car-details.html" class="fs-14 fw-semibold">${value.name}</a></h6>
									<p>${value.car_type.name}</p>
								</div>
							</div>
                            <h6 class="fw-medium"><a href="#"></a></h6></td>
                            <td>${value.main_location.name}</td>
                            <td>$${value.vehicle_basekm}</td>
                            <td>0${value.main_location_id}</td>
                            ...
                            value.id
                        });" data-bs-toggle="modal" data-bs-target="#delete-modal"><i class="ti ti-trash me-1"></i>Delete</a>
                                        </li>
                                    </ul>
                                </div>
                            </td>
    ...
    $(document).ready(function () {
        ...
        $(document).on("click", ".delete-image", function () {
            ...
            // If the image is a newly uploaded file (not yet saved in the database)
            ...
            // If it's an existing image from the database, send AJAX request
            $.ajax({
                url: "/admin/vehicle/image/delete",
                type: "POST",
                data: {
                    vehicle_id: $("#vehicle_id").val(), // Hidden input field with vehicle_id
                    image_path: fileName, // Send image path
                },
                headers: {
