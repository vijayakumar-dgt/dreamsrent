...
$(document).ready(function(){
    ...
    $("#localizationForm").validate({
        ...
        messages:{
            ...
            decimal_seperator: {
                required:  _l('admin.general_settings.decimal_seperator_required'),
            },
            thousand_seperator: {
                required:   _l('admin.general_settings.Thousand_seperator_required'),
            },
        },
        ...
        //    $("#localizationForm .submitbtn").text('Please Wait...');
            ...
            $.ajax({
                ...
                contentType: false,
                beforeSend: function () {
                    $('#localizationForm .submitbtn').attr('disabled', true).html(`
                        <span class="spinner-border spinner-border-sm align-middle" role="status" aria-hidden="true"></span> ${_l('admin.common.saving')}..
                    `);
                },
                complete: function () {
                    $('#localizationForm .submitbtn').attr('disabled', false).html(_l('admin.common.save_changes'));
                },
    ...
    $(document).on('change', '#currency', function(){
        ...
        if(currency && symbol){
        $("#currency_symbol").append(`<option value="${currency}">${symbol}</option>`);
        $("#currency_position").append(`<option value="before">${symbol}100</option><option value="after">100${symbol}</option>`);
        }else{
        $("#currency_symbol").append(`<option value="">${_l('admin.general_settings.select')}</option>`);
        }
        ...
        return new Promise((resolve, reject) => {
            $.ajax({
                ...
                success: function(resp){
                    if (resp.code === 200) {
                    $("#timezone").empty();
                    $("#timezone").append(`<option value="">${_l('admin.general_settings.select')}</option>`);
                    $("#timezone").append(`<option value="${resp.data.id}">${resp.data.name}</option>`);
                ...
                success:function(resp){
                      if (resp.code === 200) {
                         ...
                         $.each(data, function(k, val) {
                             switch (val.key) {
                                 case "timezone":
                                     $("#timezone").val(val.value).trigger("change");
                                     break;
                                 case "week_start_day":
                                     $("#week_start_day").val(val.value).trigger("change");
                                     break;
                                 case "date_format":
                                     $("#date_format").val(val.value).trigger("change");
                                     break;
                                 case "time_format":
                                     $("#time_format").val(val.value).trigger("change");
                                     break;
                                 case "default_language":
                                     $("#default_language").val(val.value).trigger("change");
                                     break;
                                 case "currency":
                                     $("#currency").val(val.value).trigger("change");
                                     break;
                                 case "currency_symbol":
                                     $("#currency_symbol").val(val.value).trigger("change");
                                     break;
                                 case "currency_position":
                                     $("#currency_position").val(val.value).trigger("change");
                                     break;
                                 case "decimal_seperator":
                                     $("#decimal_seperator").val(val.value).trigger("change");
                                     break;
                                 case "thousand_seperator":
                                     $("#thousand_seperator").val(val.value).trigger("change");
                                     break;
                                  case "currency_switcher":
                                     if(val.value == 1){
                                        $("#currency_switcher").prop("checked", true);
                                     }else{
                                        $("#currency_switcher").prop("checked", false);
                                     }
                                     break;
                                  case "language_switcher":
                                     if(val.value == 1){
                                        $("#language_switcher").prop("checked", true);
                                     }else{
                                        $("#language_switcher").prop("checked", false);
                                     }
                                     break;
                             }
                         });
