...
$("#deleteTag").on('submit', function(e){
    e.preventDefault();
    $.ajax({
        type:"POST",
        url:"/admin/delete_tag",
        data:$("#deleteTag").serialize(),
        ...
        success:function(response){
            ...
            if (response.code === 200 && response.data.length > 0) {
                ...
                $.each(data, function(index, value) {
                    tableBody += `<tr>
                            <td><h6 class="fw-medium"><a href="#">${value.tag}</a></h6></td>
                                ...
                                <div class="dropdown">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-end p-2">
                                     ${ hasPermission(permissions, 'vehicle_attributes', 'edit') ? 
                                       `<li>
                                            <a class="dropdown-item rounded-1" href="javascript:void(${value.id});" onclick="editTag(${value.id});"><i class="ti ti-edit me-1"></i>${_l('admin.common.edit')}</a>
                                        </li>`:''}
                                         ${ hasPermission(permissions, 'vehicle_attributes', 'delete') ? 
                                        `<li>
                                            <a class="dropdown-item rounded-1" href="javascript:void(${value.id});" onclick="deleteTag(${value.id});" data-bs-toggle="modal" data-bs-target="#delete-modal"><i class="ti ti-trash me-1"></i>${_l('admin.common.delete')}</a>
                                        </li>`:''}
                                    </ul>
                                </div>
                            </td>` :''
    ...
    $.ajax({
        ...
        success:function(response){
         if(response.code === 200){
             ...
             if(data.status === 1){
                 $("#add_tag #status").prop('checked', true);
             }else{
                 $("#add_tag #status").prop('checked', false);
             }

             $("#add_tag .modal-title").text(_l('admin.rentals.edit_tag'));
             $("#add_tag .submitbtn").text(_l('admin.common.save_changes'));
             $("#status_div").show();
             $("#add_tag").modal('show');
             $(".error-text").text("");
             $(".form-control").removeClass("is-invalid is-valid");
