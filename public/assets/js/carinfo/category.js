...
(async () => {
            ...
            success: function (response) {
                ...
                if (response.code === 200 && response.data.length > 0) {
                    ...
                    $.each(data, function (index, value) {
                        tableBody += `<tr>
                                <td>${value.name.length > 24 ? value.name.substring(0, 24) + "..." : value.name}</td>
                                <td>
                                ...
                                `<td>
                                    <div class="dropdown">
                                        ...
                                        ${hasPermission(permissions, 'vehicle_attributes', 'edit') ?
                                            `<li>
                                                <a class="dropdown-item rounded-1 editcategory" href="javascript:void(0);" data-id="${value.id}"><i class="ti ti-edit me-1"></i>${_l('admin.common.edit')}</a>
                                            </li>`:''}
                                            ${hasPermission(permissions, 'vehicle_attributes', 'delete') ?
                                            `<li>
                                                <a class="dropdown-item rounded-1 delateCategory" href="javascript:void(0);" data-id="${value.id}" data-bs-toggle="modal" data-bs-target="#delete-modal"><i class="ti ti-trash me-1"></i>${_l('admin.common.delete')}</a>
                                            </li>`:''}
                                        </ul>
                                    </div>
                                </td>`:''}
    ...
    $("#delateCategory").on("submit", function (e) {
        e.preventDefault();
        $.ajax({
    ...
    $(document).ready(function () {
        ...
        $("#bulkDeleteBtn").on("click", function () {
            ...
            $.ajax({
                url: "/admin/category/delete-bulk",
                type: "POST",
                data: {
