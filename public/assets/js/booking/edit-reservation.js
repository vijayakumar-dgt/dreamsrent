...
$(document).ready(function() {
    ...
    if($('.start_date').length > 0 ){
		$('.start_date').datetimepicker({
            ...
            let selectedDate = e.date ? e.date.format('DD/MM/YYYY') : null;
            let todayDate = moment().format('DD/MM/YYYY');
    
            if (selectedDate === todayDate) {
                $('.start_time').data("DateTimePicker").minDate(moment().startOf('minute'));
            } else {
    ...
    if($('.end_date').length > 0 ){
		$('.end_date').datetimepicker({
            ...
            if (selectedDate === todayDate) {
                $('.end_time').data("DateTimePicker").minDate(moment().startOf('minute'));
            } else {
    ...
    $.ajax({
        ...
        success: function(response){
            ...
            if (response.code === 200 && response.data) {
                ...
                $('#return_location').val(data.return_location).trigger('change');
                $('#pickup_location').val(data.pickup_location).trigger('change');
                $('#customer_id').val(data.customer_id).trigger('change');
                if (data.driving_type) {
                    $('#driving_type').val(data.driving_type).trigger('change');
                }
                
                let startDateTime = moment(data.start_datetime, "YYYY-MM-DD HH:mm:ss");
                startDate = startDateTime.format("DD-MM-YYYY");
                startTime = startDateTime.format("HH:mm");
                $('#start_date').val(startDate);
                $('#start_time').val(startTime);

                let endDateTime = moment(data.end_datetime, "YYYY-MM-DD HH:mm:ss");
    ...
    $.ajax({
        ...
        success: function(response){
            ...
            if (response.code === 200 && response.data.length > 0) {
                ...
                colorList.append(`<li class="text-center p-2">${_l('admin.common.no_data_found')}</li>`);
            }
    ...
    $.ajax({
        ...
        success: function(response){
            ...
            if (response.code === 200 && response.data.length > 0) {
                ...
                modelList.append(`<li class="text-center p-2">${_l('admin.common.no_data_found')}</li>`);
            }
    ...
    $.ajax({
        ...
        success: function(response){
            ...
            if (response.code === 200 && response.data.length > 0) {
                ...
                brandList.append(`<li class="text-center p-2">${_l('admin.common.no_data_found')}</li>`);
            }
    ...
    $.ajax({
        ...
        success: function(response){
            ...
            if (response.code === 200 && response.data.length > 0) {
                ...
                typeList.append(`<li class="text-center p-2">${_l('admin.common.no_data_found')}</li>`);
            }
...
$(document).on('change', '#tariff', function() {
    ...
    if (tariff === 'daily') {
        lastDateTime = todayDateTime.clone().add(1, 'days').subtract(1, 'seconds');
        $('#start_date, #end_date, #start_time, #end_time').attr('readonly', false);
    } else if (tariff === 'weekly') {
        lastDateTime = todayDateTime.clone().add(1, 'weeks').subtract(1, 'seconds');
        $('#start_date, #end_date, #start_time, #end_time').attr('readonly', true);
    } else if (tariff === 'monthly') {
        lastDateTime = todayDateTime.clone().add(1, 'months').subtract(1, 'seconds');
        $('#start_date, #end_date, #start_time, #end_time').attr('readonly', true);
    } else if (tariff === 'yearly') {
        lastDateTime = todayDateTime.clone().add(1, 'years').subtract(1, 'seconds');
        $('#start_date, #end_date, #start_time, #end_time').attr('readonly', true);
    }
    ...
    $.ajax({
        ...
        success: function (result) {
            if (result.data && result.data.data.length > 0) {
                ...
                let options = data.map(function (item) {
                    let is_checked = item.id == edit_vehicle_id ? 'checked' : '';

                    return `
                        <div class="card vehicle-card" id="vehicle_${item.id}" data-image="${item.image}" data-type="${item.vehicle_type}" data-name="${item.vehicle_name}" data-price="${item.vehicle_price}" data-price_type="${item.vehicle_price_type}">
                            <div class="card-body">
                                <div class="row gy-3">
                                    <div class="col-3">
                                        <div class="d-flex align-items-center">
                                            <div class="form-check form-check-md me-3">
                                                <input class="form-check-input vehicle_select" name="vehicle_id" id="vehicle_select_${item.id}" value="${item.id}" type="radio" data-price="${item.vehicle_price}" data-price_type="${item.vehicle_price_type}" data-vehicle_name="${item.vehicle_name}" data-vehicle_season_id="${item.vehicle_season_id ?? ''}" data-vehicle_tariff_id="${item.vehicle_tariff_id ?? ''}" ${is_checked}>
                                            </div>
                                            <span class="avatar flex-shrink-0 me-2">
                                                <img src="${item.image}" alt="">
                                            </span>
                                            <div>
                                                <p class="mb-1">${item.vehicle_type}</p>
                                                <h6 class="fs-14">${item.vehicle_name}</h6>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-6">
                                        <div class="d-flex gy-3 gap-5">
                                            <div class="">
                                                <div>
                                                    <p class="mb-1">${_l('admin.common.color')}</p>
                                                    <h6 class="fs-14 d-inline-flex align-items-center">
                                                        <i class="ti ti-square-filled me-1" style="color: ${item.color_value};"></i>${item.color_name}
                                                    </h6>
                                                </div>
                                            </div>
                                            <div class="">
                                                <div>
                                                    <p class="mb-1">${_l('admin.common.year')}</p>
                                                    <h6 class="fs-14">${item.year}</h6>
                                                </div>
                                            </div>
                                            <div class="">
                                                <div>
                                                    <p class="mb-1">${_l('admin.common.price')}</p>
                                    ...
                                    <div class="col-3">
                                        <div class="float-md-start">
                                            <span class="badge bg-orange-transparent d-inline-flex align-items-center badge-sm mb-1">
                                                <i class="ti ti-point-filled me-1"></i>${_l('admin.common.available')}
                                            </span>
                                            <h6 class="fs-14">${item.model_name}</h6>
                                        </div>
                                    </div>
                                </div>
                            </div>
        ...
        error: function (error) {
            if (error.responseJSON.code === 500) {
                ...
                showToast('errror', _l('admin.common.default_retrieve_error'));
            }
        },
        complete: function () {
            isFetching = false;
            $(".list-loader, .card-loader").hide();
            $('#vehicle_list_container').removeClass('d-none');
        }
    });
}
...
$('#vehicle_list_container').on('scroll', function () {
    ...
    if (scrollTop + containerHeight >= scrollHeight - threshold && !isFetching) {
            ...
            if (response.code === 200 && response.data) {
                ...
                $('#driver_details_list').html(`
                    <div class="d-flex align-items-center justify-content-end mb-3">
                        ...
                        data-phone="${data.phone_number}" data-price="${edit_driver_price != '' ? edit_driver_price : 0}">${_l('admin.bookings.edit_price')}</a>
                    </div>
        ...
        success: function (result) {
            if (result.data && result.data.length > 0) {
                ...
                let options = data.map(item => {
                    return `<option value="${item.id}" ${(item.id == selected_driver_id ) ? 'selected' : ''}>${item.driver_name}</option>`;
                }).join('');
...
$("#basicInfoForm").validate({
    ...
    messages:{
        start_date: {
            ...
            dateLessThan: _l('admin.bookings.start_date_less_than_equal_to_end_date'),
        },
        start_time: {
        ...
        end_date: {
            ...
            dateGreaterThan: _l('admin.bookings.end_date_greater_than_equal_to_start_date'),
        },
        end_time: {
    ...
    if ($("#basicInfoForm").valid()) {
        if(selected_vehicle_ids.length > 0) {
            ...
            $.each(selected_vehicle_ids, function (index , item) {
                ...
                let vehicle_price_type = $('#vehicle_' + item).data('price_type');
                ...
                $('#customer_summary').append(`
                    <div class="border rounded p-3 bg-light mb-3">
                        <div class="row">
                            ...
                            <div class="col-6">
                                <div class="text-end">
                                    <p class="mb-0">${_l('admin.common.price')}</p>
...
function calculateVehiclePrice() {
    let tariffVal = $('#tariff').find("option:selected").text().toLowerCase();
    let vehiclePriceRate = vehiclePrice;
    var driverPriceVal = parseFloat($('#driver_price').val()).toFixed(2) || 0;
    var securityDeposit = $('#security_deposit').val() !== '' ? parseFloat($('#security_deposit').val()).toFixed(2) : 0;
    var response = [];
        ...
        $('.vehicle_extra_service').each(function () {
            ...
            if(isExtraService) {
                if ($(this).data('price_type') == 'per_day') {
                    ...
                    total_extra_service_price += (vehiclePriceRate * parseFloat($(this).data('price'))) / 100;
                    total_extra_service++;
                } 
        ...
        $('.vehicle_insurance').each(function () {
            if($(this).is(':checked')) {
                if ($(this).data('price_type') == 'daily') {
                    total_insurance_price += (no_of_days * parseFloat($(this).data('price')));
                    total_insurance++;
                } else if ($(this).data('price_type') == 'fixed') {
                    total_insurance_price += parseFloat($(this).data('price'));
                    total_insurance++;
                } else if ($(this).data('price_type') == 'percentage') {
                    total_insurance_price += (vehiclePriceRate * parseFloat($(this).data('price'))) / 100;
                    total_insurance++;
                } 
        ...
        $('#extra_service_summary').append(`
            <div class="border-bottom mb-3 border-top pt-3">
                <div class="d-flex align-items-center justify-content-between">
                    <div class="mb-3">
                        <h6 class="d-inline-flex align-items-center fs-14 fw-medium ">${_l('admin.common.customer')}<a href="#" class="ms-2 d-none"><i class="ti ti-edit"></i></a></h6>
                    </div>
                ...
                <div class="d-flex align-items-center justify-content-between ${typeof driver_name !== 'undefined' && driver_name !== '' ? '' : 'd-none'}">
                    <div class="mb-3">
            ...
            <div class="border-bottom mb-3 pb-2" id="price_container">
                <div class="d-flex align-items-center justify-content-between mb-2">
                    ...
                    <p>${default_currency}${totalAmount[0]['vehicle_price_rate']}</p>
                </div>
                <div class="d-flex align-items-center justify-content-between mb-2">
                    <h6 class="fw-medium d-flex align-items-center fs-14"> <span class="extra_service_count me-1">${totalAmount[0]['total_extra_service']} </span> ${_l('admin.common.extra_services')}
                        <a href="javascript:void(0);" class="me-2 ms-2 extra_service_tooltip" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-original-title="${totalAmount[0]['extra_service_name']}"><i class="ti ti-info-circle-filled"></i></a>
                        <a href="javascript:void(0);" class="d-none"><i class="ti ti-edit"></i></a>
                    </h6>
                    <p>${default_currency}<span class="extra_service_price">${totalAmount[0]['total_extra_service_price']}</span></p>
                </div>
                <div class="d-flex align-items-center justify-content-between mb-2">
                    <h6 class="fw-medium d-flex align-items-center fs-14">${_l('admin.bookings.security_deposit')}
                        <a href="javascript:void(0);" class="ms-2 d-none"><i class="ti ti-edit"></i></a>
                    </h6>
                    <p>${default_currency}${totalAmount[0]['security_deposit']}</p>
                </div>
                <div class="d-flex align-items-center justify-content-between mb-2 ${typeof driver_name !== 'undefined' && driver_name !== '' ? '' : 'd-none'}">
                    <h6 class="fw-medium d-flex align-items-center fs-14">${_l('admin.bookings.driver_price')}
                        <a href="javascript:void(0);" class="ms-2 d-none"><i class="ti ti-edit"></i></a>
                    </h6>
                ...
                <div class="d-flex align-items-center justify-content-between mb-2 d-none insurance_summary_container">
                    <h6 class="fw-medium d-flex align-items-center fs-14"> <span class="insurance_count me-1">${totalAmount[0]['total_insurance']} </span> ${_l('admin.common.insurances')}
                        <a href="javascript:void(0);" class="me-2 ms-2 insurance_tooltip" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-original-title="${totalAmount[0]['insurance_name']}"><i class="ti ti-info-circle-filled"></i></a>
                        <a href="javascript:void(0);" class="d-none"><i class="ti ti-edit"></i></a>
                    </h6>
                    <p>${default_currency}<span class="total_insurance_price">${totalAmount[0]['total_insurance_price']}</span></p>
                </div>
            </div>
            <div class="d-flex align-items-center justify-content-between">
                <h6>${_l('admin.common.total_price')}</h6>
                <h6 class="total_price">${default_currency}<span class="total_price_val">${totalAmount[0]['total_price']}</span></h6>
            </div>
        `);
    ...
    $.ajax({
        ...
        success: function (result) {
            if (result.data && result.data.length > 0) {
                ...
                let options = data.map(item => {
                    let isChecked = (selected_extra_service_ids.includes(item.id.toString()) || 
                        edit_extra_service.find(service => service.id == item.id)) ? 'checked' : '';

                    let isActive = (selected_extra_service_ids.includes(item.id.toString()) || 
                            edit_extra_service.find(service => service.id == item.id)) ? 'active' : '';
                        ...
                        <div class="col-md-6">
                            <div class="custom-checkbox ${isActive}">
                                <div class="form-check form-check-md">
                                    <input class="form-check-input vehicle_extra_service" type="checkbox" name="extra_services[]" value="${item.id}" id="extra-service-${item.id}" ${isChecked} data-price="${item.price}" data-price_type="${item.extra_service_type}" data-name="${item.name}">
                                </div>
                                <div class="d-flex align-items-center justify-content-between">
                                    <label class="form-check-label ms-2 ps-4" for="extra-service-${item.id}">
                                        <span class="fw-semibold text-gray-9 d-block mb-1">${item.name}</span>
                                        <span class="d-block">${item.description}</span>
                                    </label>
                                    <div class="text-end">
                                        <p class="mb-1">${extraServiceType}</p>
                                        ${ item.extra_service_type == 'percentage' ?
...
$(document).on('click', '#reservation_complete_btn', function (event) {
    ...
    $('.vehicle_insurance').each(function () {
        ...
        let vehicle_tariff_id = $('.vehicle_select:checked').data('vehicle_tariff_id');
        ...
        finalFormData.append('extra_service', JSON.stringify(extraServicePayload));
        finalFormData.append('insurance', JSON.stringify(insurancePayload));
        finalFormData.append('total_insurance_price', response[0]['total_insurance_price']);
        finalFormData.append('total_extra_service_price', response[0]['total_extra_service_price']);
        ...
        finalFormData.append('vehicle_total_price', response[0]['vehicle_price_rate']);
        ...
        $.ajax({
            ...
            success: function (response) {
                $('#reservation_complete_btn').removeAttr('disabled').html(`${_l('admin.common.finish')} & ${_l('admin.common.save')} <i class="ti ti-chevron-right ms-1"></i>`);
                if (response.code === 200) {
                    $('#final_vehicle_name').html(vehicle_name);
                    $('#final_reservation_date').html(moment().format('DD MMM YYYY'));
