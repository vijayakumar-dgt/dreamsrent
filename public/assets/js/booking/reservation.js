...
function getPickupLocation(search = '') {
    $.ajax({
        url: "/get-locations",
        type: "POST",
        data: {
        ...
        success: function(response){
            let pickup_location_wrapper = $('#pickUpLocationList .custom-scroll');
            pickup_location_wrapper.empty();

            if (response.code === 200 && response.data.length > 0) {
                let data = response.data;

                $.each(data, function(index, value) {
                    pickup_location_wrapper.append(`
                        <li>
                            <label class="dropdown-item d-flex align-items-center rounded-1">
                                <input class="form-check-input m-0 me-2 pickup_location_checkbox" type="checkbox" value="${value.id}">${value.name}
                            </label>
                        </li>
                    `);
                });
                ...
                pickup_location_wrapper.append(`<li class="text-center p-2">${_l('admin.common.no_data_found')}</li>`);
            }
...
function getDropLocation(search = '') {
    $.ajax({
        url: "/get-locations",
        type: "POST",
        data: {
        ...
        success: function(response){
            let drop_location_wrapper = $('#dropOffLocationList .custom-scroll');
            drop_location_wrapper.empty();

            if (response.code === 200 && response.data.length > 0) {
                let data = response.data;

                $.each(data, function(index, value) {
                    drop_location_wrapper.append(`
                        <li>
                            <label class="dropdown-item d-flex align-items-center rounded-1">
                                <input class="form-check-input m-0 me-2 drop_location_checkbox" type="checkbox" value="${value.id}">${value.name}
                            </label>
                        </li>
                    `);
                });
                ...
                drop_location_wrapper.append(`<li class="text-center p-2">${_l('admin.common.no_data_found')}</li>`);
            }
                ...
                return `
                    <div class="dropdown">
                        ...
                        <ul class="dropdown-menu dropdown-menu-end p-2">
                            <li>
                                <a class="dropdown-item rounded-1" href="/admin/reservation-details/${row.encrypted_id}w"><i class="ti ti-eye me-1"></i>${_l('admin.common.view_details')}</a>
                            </li>
                            ...
                            `<li>
                                <a class="dropdown-item rounded-1" href="javascript:void(0);" onclick="deleteReservation(${row.id});" data-bs-toggle="modal" data-bs-target="#delete_modal"><i class="ti ti-trash me-1"></i>${_l('admin.common.delete')}</a>
                            </li>`:''}
                        </ul>
                    </div>
                `;
