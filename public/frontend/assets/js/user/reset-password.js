...
(async () => {
  ...
  $(function () {
    ...
    function handleChangePasswordSuccess(resp) {
        if (resp.code === 200) {
    ...
    // AJAX error handler
    function handleChangePasswordError(xhr) {
        $(".error-text").text("");
    ...
    function handleChangePasswordComplete() {
    ...
    function submitChangePasswordForm(form) {
        ...
        $btn.html('<div class="spinner-border spinner-border-sm text-light" role="status"></div>')
            .prop("disabled", true);
        ...
        $.ajax({
            ...
            complete: handleChangePasswordComplete
        });
    }
    ...
    $form.validate({
        rules: {
            ...
            confirm_password: { required: true, equalTo: "#current_password" }
        },
        messages: {
            current_password: {
