...
(async () => {
    ...
    $(document).ready(() => {
                ...
                success: function (response) {
                    ...
                    if (data.length > 0) {
                        data.forEach(function (value) {
                            ...
                            const assigneeDetail = assignee && assignee.user_detail;
                            const assigneeName = assigneeDetail
                                ? assigneeDetail.first_name + " " + assigneeDetail.last_name
                                : assignee
                                ? assignee.name
                                : _l("web.user.unassigned");
                            const assigneeImage = assigneeDetail?.profile_image
                                ? "/storage/" + assigneeDetail.profile_image
        ...
        function showTicketHistory(ticketId) {
            const ticket = ticketData.find(function (t) { return t.id === ticketId; });
            const $historyContainer = $(".ticket_histroy").empty();

            if (!ticket || !ticket.ticket_histories || ticket.ticket_histories.length === 0) {
            ...
            ticket.ticket_histories.forEach(function (history) {
                const userDetail = history.user?.user_detail;
                const userImage = userDetail?.profile_image
                    ? "/storage/" + encodeURIComponent(userDetail.profile_image)
