...
(async () => {
    ...
    $(() => {
        ...
        const fetchReviews = (sortBy = getSortBy()) => {
            $("#reviewsTable").DataTable({
                ...
                columns: [
                    ...
                    {
                        data: "comments",
                        render: (data, type, row) => `<p>${truncateText(row.comments)}</p>`
                    },
                    {
                        data: "average_ratings",
                        render: (data, type, row) => {
                            const starsHtml = createStarsHtml(parseFloat(row.average_ratings));
        ...
        $("#reviewDeleteForm").on("submit", async (e) => {
            ...
            try {
                ...
                const msg = error.responseJSON?.code === 500
                    ? error.responseJSON.message
                    : _l("web.common.default_delete_error");
                showToast("error", msg);
            }
        });
